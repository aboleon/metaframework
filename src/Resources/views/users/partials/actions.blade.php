<ul class="mfw-actions">
    ...
    @else
        <x-mfw::delete-modal-link reference="{{ $data->getKey() }}" :title="__('mfw.archive_this')"/>
    @endif
</ul>
...
@if($supportsSoftDeletes && method_exists($data, 'trashed') && $data->trashed())
    <x-mfw::modal
        ...
        :question="__('mfw.restore') . ' - ' . (method_exists($data, 'names') ? $data->names() : ($data->email ?? $data->getKey())) . ' ?'"
        reference="restore_{{ $data->getKey() }}"
    />
@else
    <x-mfw::modal
        :route="route('mfw.users.destroy', $data->getKey())"
        :question="__('mfw.should_i_delete_user') . ' - ' . (method_exists($data, 'names') ? $data->names() : ($data->email ?? $data->getKey())) . ' ?'"
