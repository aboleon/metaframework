...
<div class="col-12">
    <h4>{{ __('mfw::mfw-users.users.password') }}</h4>

    @if($isEdit)
        <div class="form-check mb-2">
            <input type="checkbox" class="form-check-input" name="password_change" id="password_change"/>
            <label class="form-label" for="password_change">{{ __('mfw::mfw-users.users.password_change') }}</label>
        </div>
    @else
    ...
    <div class="form-check mb-2">
        ...
        <label class="form-label" for="random_password">{{ __('mfw::mfw-users.users.password_random') }}</label>
    </div>
</div>

<div class="col-12 col-xl-6 mt-3">
    <div class="form-group">
        <label class="form-label" for="password">{{ __('mfw::mfw-users.users.password_new') }}</label>
        <input id="password" type="password" name="password" class="form-control" value="" @if($isEdit) disabled @endif/>
        <span>{{ __('mfw::mfw-users.users.password_min_chars', ['min' => 8]) }}</span>
    </div>
</div>
<div class="col-12 col-xl-6 mt-3">
    <div class="form-group">
        <label class="form-label" for="password_confirmation">{{ __('mfw::mfw-users.users.password_repeat') }}</label>
